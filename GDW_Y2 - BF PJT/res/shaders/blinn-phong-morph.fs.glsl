...
void main() {
    ...
    int lightsTotal = clamp(a_EnabledLights, 0, MAX_LIGHTS);

    // getting all results
    for(int i = 1; i <= lightsTotal; i++)
    {
        finalResult.rgb += ComputeBlinnPhong(a_Lights[i - 1], inNormal, albedo);
	}

    // averaging the final result.
    finalResult.x /= lightsTotal;
    finalResult.y /= lightsTotal;
    finalResult.z /= lightsTotal;
