...
PrimitiveSphere::PrimitiveSphere(float radius, unsigned int segRows, unsigned int segCols) : Primitive()
{
	...
	uint32_t p0 = 0, p1 = 0, p2 = 0, p3 = 0; // points used to pull values from the indiArr
	...
	for (unsigned int row = 0; row < segRows && index < verticesTotal - 1; row++) // goes through each row
	{
	...
	index = 0;
	int test = 0;
	for (unsigned int row = 0; row < segRows - 1; row++)
	{
		for (unsigned int col = 0; col < segCols; col++)
		{
			...
			else if (row == segRows - 2) // final row
			{
