#include "EngineGameplayScene0.h"
...
cherry::EngineGameplayScene0::EngineGameplayScene0(std::string sceneName) : GameplayScene(sceneName)
{
}

void cherry::EngineGameplayScene0::OnOpen()
{
	// starts up profiling
	if (PROFILE)
	...
	if (true)
	{
		...
		float offset = 4.0F; // position offset

		// Creating the sceneLists, storing them, and making them part of the default m_Scene.
		objectList->objects.push_back(new PrimitiveCapsule(2.0F, 13.0F, 17, 12));
		objectList->objects.at(objectList->objects.size() - 1)->CreateEntity(game->GetCurrentSceneName(), matStatic);
		objectList->objects.at(objectList->objects.size() - 1)->SetPosition(-offset, -offset, 1.0F);
		objectList->objects.at(objectList->objects.size() - 1)->SetVisible(true);

		objectList->objects.push_back(new PrimitiveCircle(5.0F, 12.0F, true, Vec4(0.4F, 0.5F, 0.9F, 1.0F)));
		objectList->objects.at(objectList->objects.size() - 1)->CreateEntity(game->GetCurrentSceneName(), matStatic);
		...
		objectList->objects.at(objectList->objects.size() - 1)->CreateEntity(game->GetCurrentSceneName(), matStatic);
		objectList->objects.at(objectList->objects.size() - 1)->SetPosition(0.0F, 0.0F, 1.0F);
		objectList->objects.at(objectList->objects.size() - 1)->SetVisible(true);
		...
		// Height Map
		{
			...
			Terrain* terrain = new Terrain(game->GetCurrentSceneName(), "res/images/heightmaps/dp_test_05.bmp", 45.0f, 70, false);
			...
			terrain->SetMaximumHeight(2.5F);
			terrain->SetPosition(0.0F, 0.0F, -12.0F);
	...
	if (USE_LAYERS)
	{
		...
		// K6 - blurs
		{
			...
			blurShader = BloomLayer::GenerateRadialBlurCircular(glm::vec2(0.5F, 0.5F), glm::radians(30.0F), 10, 1);
			blurBuffer = FrameBuffer::GenerateDefaultBuffer();

			layers5.push(std::make_shared<PostLayer>(blurShader, blurBuffer));

			// radial blur - linear
			blurShader = BloomLayer::GenerateRadialBlurLinear(5.0F, glm::radians(30.0F), 10);
			blurBuffer = FrameBuffer::GenerateDefaultBuffer();

			layers5.push(std::make_shared<PostLayer>(blurShader, blurBuffer));

			// radial - zoom in
			blurShader = BloomLayer::GenerateRadialBlurZoom(glm::vec2(0.5F, 0.5F), 7.0F, 10);
			blurBuffer = FrameBuffer::GenerateDefaultBuffer();
...
void cherry::EngineGameplayScene0::OnClose()
{
...
void cherry::EngineGameplayScene0::MouseButtonPressed(GLFWwindow* window, int button)
{
	// checks each button
	switch (button) {
...
void cherry::EngineGameplayScene0::MouseButtonHeld(GLFWwindow* window, int button)
{
	// checks each button
	switch (button) {
	case GLFW_MOUSE_BUTTON_LEFT:
...
void cherry::EngineGameplayScene0::MouseButtonReleased(GLFWwindow* window, int button)
{
	// checks each button
	switch (button) {
	case GLFW_MOUSE_BUTTON_LEFT:
...
void cherry::EngineGameplayScene0::KeyPressed(GLFWwindow* window, int key)
{
...
// key held
void cherry::EngineGameplayScene0::KeyHeld(GLFWwindow* window, int key)
{
	Game* game = (Game*)glfwGetWindowUserPointer(window);
...
// key released
void cherry::EngineGameplayScene0::KeyReleased(GLFWwindow* window, int key)
{
	Game* game = (Game*)glfwGetWindowUserPointer(window);
...
// imgui draw function
void cherry::EngineGameplayScene0::DrawGui(float deltaTime)
{
...
// update loop
void cherry::EngineGameplayScene0::Update(float deltaTime)
{
	Game* const game = Game::GetRunningGame();
